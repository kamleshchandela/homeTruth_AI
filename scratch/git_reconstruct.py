...
prs = [
    ...
    ("feat/sell-home-integration", "feat: connect sell form to api", ["."]) # Ading ALL remaining files here
]
...
def main():
    ...
    # 2. Init and configure git locally
    run("git init")
    run("git config user.name \"Simulation Bot\"")
    run("git config user.email \"bot@example.com\"")
    
    # 3. Create initial empty commit on main
    run("git checkout -b main", allow_fail=True)
    ...
    for i, (branch, msg, files) in enumerate(prs, 1):
        ...
        has_files = False
        for f in files:
            # We don't want to error out if a file doesn't exist just in case
            res = run(f"git add {f}", allow_fail=True)
            has_files = True
            
        # Commit
        if has_files:
            # Commit even if there's nothing new to commit, using --allow-empty just in case
        ...
        run(f"git merge --no-ff {branch} -m \"Merge pull request #{i} from {branch}\n\n{msg}\"")
        # run(f"git branch -d {branch}") # We'll keep branches so it looks good, or delete them. Let's keep them!
        
    print("\nGit History Reconstruction Complete!")
